...
RUN apt-get install -y qemu-system #software-properties-common
...
RUN groupadd -g 1001 tuesday && groupadd -g 1002 gptenv && useradd -m -u 1001 -g 1001 -G users,gptenv,sudo,tty,video,mail -s /bin/bash tuesday && mkdir /app && \
...
#RUN source ~/.profile && pip install gptme # run these pip install commands on the host
