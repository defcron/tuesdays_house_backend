    ...
    try:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile("w", delete=False, suffix=".sh") as f:
                ...
                f.write(f'tmux send-keys -t "{tmuxSession}-$THB_CMD_TIMESTAMP" "date" C-m\n')
                f.write(f'THB_MAX_WAIT_SECONDS=25\n')
                f.write(f'THB_WAIT_STARTED_AT="$(date +%s)"\n')
                f.write(f'while [ ! -f "{command_exit_code_file_path}" ]; do\n')
                f.write(f'  THB_NOW="$(date +%s)"\n')
                f.write(f'  if [ $((THB_NOW - THB_WAIT_STARTED_AT)) -ge "$THB_MAX_WAIT_SECONDS" ]; then\n')
                f.write(f'    break\n')
                f.write(f'  fi\n')
                f.write(f'  sleep 0.25\n')
                f.write(f'done\n')
                f.write(f'tmux capture-pane -t "{tmuxSession}-$THB_CMD_TIMESTAMP" -pS -\n')
